...
public:
	static double going(int n){
		if (n < 1) throw std::invalid_argument("n is an integer greater or equal to 1");
		constexpr auto epsilon = std::numeric_limits<double>::epsilon(); 
		double result{1}, term{1};
		for (auto i = n; i > 1; --i){
			term /= i;
			result += term;
			if (term < epsilon) break;	// below rounding error
		}
		// truncate to 6 decimals. Not all decimals have binary representation!
  		return std::trunc(1000000*result)/1000000;
  }
};
