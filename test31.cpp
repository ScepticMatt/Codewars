...
public:
	static std::string arrange(const std::string &strng){
		...
		std::istringstream is(strng);	// extract white space separated strings
		std::string output;
		output.reserve(strng.length());
		...
		while (is >> second){
			...
			set_case(isGreater, first);
			output+=first+' ';
			isGreater ^= true;	// fip sign and case
			swap(first,second);
		}
		set_case(isGreater, first);
		output+=first;
...
private:
	static void set_case(bool isUpper, std::string& str){
		if (isUpper)
...
int main(){
	std::cout << UpAndDown::arrange("after be arrived two My so");
}